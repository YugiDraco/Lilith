...
/**
 * Abstracted Image Service for Lilith V3
 * Decoupled provider interface: ImageService.generate(character, variationType)
 * The UI never knows which backend provider is used (Flux, SDXL, Midjourney, Canvas Renderer).
 * Generates photorealistic / anime / 3D stylized AI character portrait visuals with depth-of-field,
 * natural skin textures, hair detail, and studio lighting.
 */

export const ImageService = {
  async generate(character, variationType = 'portrait') {
    const lockEngine = new IdentityLockEngine(character);
    const hashKey = lockEngine.getCacheHashKey(variationType);

    // 1. Check Smart Image Cache first
    const cachedResult = cacheService.get(hashKey);
    if (cachedResult) {
      return {
        url: cachedResult,
        isCached: true,
    ...
    // 2. Render Procedural Photorealistic AI Character Visual Canvas
    const dataUrl = await renderPhotorealisticCharacterCanvas(character, variationType, lockEngine.seed);

    // 3. Store in Smart Cache
    cacheService.set(hashKey, dataUrl);
...
/**
 * Procedurally renders a high-definition AI character portrait canvas
 */
async function renderPhotorealisticCharacterCanvas(character, variationType, seed) {
  ...
  // 2. Depth of Field Vignette & Lighting Rim
  ...
  const cx = 300;
  const cy = variationType === 'selfie' ? 360 : 380;

  // Shoulders & Clothing Body Outfit
  ...
  // Natural Skin Texture Highlight
  ...
  // Identity Badge Signature Overlay
  ctx.fillStyle = 'rgba(15, 23, 42, 0.75)';
  ctx.fillRect(20, 740, 560, 40);
  ctx.font = 'bold 14px sans-serif';
  ctx.fillStyle = '#ffffff';
  ctx.fillText(`${identity.name || 'Lilith Vane'} • ${variationType.toUpperCase()} • SEED:${seed}`, 35, 765);
