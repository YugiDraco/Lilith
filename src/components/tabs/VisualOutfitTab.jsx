...
import { CLOTHING_CATEGORIES } from '../../types/characterV2';
import { Shirt, Palette, Sparkles } from 'lucide-react';

export default function VisualOutfitTab({ character, onChange }) {
  const { clothing = { activeCategory: 'Sci-Fi', outfits: {} } } = character;
  const activeCat = clothing.activeCategory || 'Sci-Fi';
  const currentOutfit = clothing.outfits?.[activeCat] || {};

  const handleSelectOutfitAsset = (asset) => {
    const cat = asset.clothingCategory || activeCat;
    onChange({
      ...character,
      clothing: {
        ...clothing,
        activeCategory: cat,
        outfitAssetId: asset.id
      }
    });
  };
  ...
  return (
    <div className="space-y-6">
      {/* Outfit Visual Gallery */}
      <VisualAssetGallery
        category="outfits"
        selectedAssetId={clothing.outfitAssetId || 'outfit_scifi_01'}
        onSelectAsset={handleSelectOutfitAsset}
        title="Visual Outfit Cards"
        description="Select complete outfit visual cards dynamically."
      />
      ...
      <div className="glass-card p-5 rounded-2xl border border-slate-800 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            <label className="text-xs text-slate-400 font-medium block mb-1">Outer Layer / Cloak</label>
            <input
              type="text"
          ...
          <div className="bg-dark-900/80 p-3 rounded-xl border border-slate-800">
            ...
            <div className="flex items-center gap-2">
              <input
                type="color"
                value={currentOutfit.primaryColor || '#090d16'}
          ...
          <div className="bg-dark-900/80 p-3 rounded-xl border border-slate-800">
            ...
            <div className="flex items-center gap-2">
              <input
                type="color"
                value={currentOutfit.secondaryColor || '#6366f1'}
